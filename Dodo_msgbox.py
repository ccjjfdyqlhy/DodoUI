...
def create_styled_window(title="Styled Window", content="This is the window content."):
    """
    创建一个风格化的窗口，根据传入的参数显示内容，并在一段时间后自动销毁。
    将 [[...]] 标识符框起来的部分文本放大显示。
    添加一个方形小按钮，按下后停止定时销毁，并将按钮文本改为 "Pinned" 并禁用按钮。
    窗口大小会根据内容自动调整。
    添加 "Proceed" 按钮，用于显示下一条句子。
    即使窗口自动销毁，也会显示下一条句子（如果不是最后一条）。
    ...
    def destroy_window():
        root.destroy()
        show_next_sentence()  # 在销毁后显示下一条句子
